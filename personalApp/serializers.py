...
class DepartmentSerializer(serializers.ModelSerializer):
    personel_count = serializers.SerializerMethodField() #? method field denir aşağıdada methodunu yazdık (get_personal_count)
    class Meta:
        model = Department
        fields = ("id", "name", "personel_count")
        
    def get_personel_count(self,obj):
        return Personal.objects.filter(department = obj.id).count() #? departmandaki personelleri saydırdık
...
class DepartmentPersonalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Department
        fields = ("id", "name", "personel_count", "departments")
